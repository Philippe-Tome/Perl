...
sub process_file {
    ...
    $/ = "</entry>";   # Record separator. This is global.

    while(my $chunk = <INPUTFILE>) {
        my ($band) = $chunk =~ m'<band>(.*?)</band>';   # ($band) is an array with one entry.

        unless(defined($band)) {
            next;
        }

        my @albums = $chunk =~ m'<album>(.*?)</album>'sg;   # ($band) is an array with one entry. s = match over multiple lines. g = multiple matches.
